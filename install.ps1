...
function Test-ControllerReady([string]$BaseUrl) {
    try {
        Invoke-WebRequest -UseBasicParsing -Uri "$BaseUrl/health/ready" -TimeoutSec 3 | Out-Null
        return $true
    } catch {
        return $false
    }
...
function Wait-ControllerReady([string]$BaseUrl, [int]$TimeoutSeconds) {
    $deadline = (Get-Date).AddSeconds($TimeoutSeconds)
    while ((Get-Date) -lt $deadline) {
        if (Test-ControllerReady $BaseUrl) { return $true }
...
if (Test-Path $EnvFile) {
    ...
    $credentialLine = Get-Content -Path $EnvFile | Where-Object { $_ -match '^\s*CREDENTIAL_ENCRYPTION_KEY\s*=' } | Select-Object -First 1
    $credentialValue = if ($credentialLine) { ($credentialLine -split '=', 2)[1].Trim() } else { '' }
...
# when present. On re-runs the stale-secret reaper may have deleted it,
# so readiness is the source of truth for already-bootstrapped installs.
...
while ((Get-Date) -lt $deadline) {
    ...
    if (-not $FreshInstall) {
        if (Test-ControllerReady $DashboardUrl) { break }
    }
    Start-Sleep -Seconds 2
}

Write-Step "Waiting for controller readiness at $DashboardUrl/health/ready (up to 120 s)…"
if (-not (Wait-ControllerReady $DashboardUrl 120)) {
    Write-Fail "Controller did not become ready. Check logs with: docker compose logs controller"
}
...
if ($AdminEmail -and $AdminPassword) {
    ...
    Write-Host "    1. Log in and rotate the password under Admin -> Profile."
    Write-Host "    2. Open $DashboardUrl/admin/settings/connect"
    Write-Host "       for your 6-digit pairing code, then install the agent on"
    Write-Host "       any endpoint from https://github.com/f7-platform/public-agent-binaries/releases/latest"
    Write-Host ""
} elseif (-not $FreshInstall) {
    Write-Host "Dashboard:  $DashboardUrl"
    Write-Host "(Admin credentials were printed on first run. If the bootstrap handoff"
    Write-Host " has not expired, retrieve them with:"
    Write-Host "   docker compose exec controller cat $SecretsPath"
    Write-Host " Otherwise use the admin password reset flow; stale bootstrap secrets"
    Write-Host " are deleted automatically after first-run handoff.)"
} else {
